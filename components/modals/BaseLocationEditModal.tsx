...
export function BaseLocationEditModal({ dayId, locationIndex, location, onClose }: BaseLocationEditModalProps) {
  const { updateBaseLocation, removeBaseLocation } = useSupabaseTripStore()
  ...
  const handleRemove = async () => {
    if (confirm('Are you sure you want to remove this base location?')) {
      setIsLoading(true)
      try {
        await removeBaseLocation(dayId, locationIndex)
        onClose()
      } catch (error) {
        console.error('Error removing base location:', error)
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-shrink-0 flex items-center justify-between p-6 border-t border-white/10">
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 transition-all duration-200 disabled:opacity-50"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500 text-white font-medium transition-all duration-200 hover:bg-green-600 disabled:opacity-50"
            >
