...
export function ItineraryOverlay() {
  ...
  return (
    <div
      className="pointer-events-none fixed left-4 z-[55] flex"
      style={{
        top: 'calc(var(--app-navbar-height, 5rem) + 1.5rem)',
        bottom: '1.5rem',
        maxHeight: 'calc(100vh - var(--app-navbar-height, 5rem) - 3rem)',
      }}
    >
      <div
        className="pointer-events-auto relative flex h-full w-[min(980px,calc(100vw-5rem))] flex-col gap-4 transition-[width] duration-300 ease-in-out"
        style={
          isCompact
            ? {
                width: `${TIMELINE_WIDTH + COLLAPSE_PEEK_WIDTH}px`,
              }
            : undefined
        }
      ...
      >
        ...
        <div
          className={clsx(
            'relative flex-1 overflow-hidden rounded-[32px] border border-white/12 bg-slate-950/80 shadow-[0_45px_120px_-40px_rgba(8,15,35,0.95)] backdrop-blur-2xl transition-colors duration-200',
            isCompact ? 'bg-slate-950/85' : 'bg-slate-950/80'
          )}
        >
          <ItineraryTab isDetailCollapsed={isCompact} />
        </div>
